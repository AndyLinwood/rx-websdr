...
- **FT8-декодер** — кнопка **FT8** в ряду режимов открывает окно декодера:
  live-декоды FT8 всех диапазонов (160..10 м) в табличном виде с автообновлением,
  фильтр по текущему диапазону, клик по строке перестраивает приёмник на частоту станции.
  Backend: секция `[FT8]` в radiod (мультикаст `ft8-pcm.local`) → pcmrecord → decode_ft8
  (ka9q/ft8_lib) → лог, сервер отдаёт хвост через `/~~ft8`.
