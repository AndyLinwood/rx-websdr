...
```
 антенна → RX888MKII (USB) → radiod (ka9q-radio) → multicast RTP (lo)
        → pcmrecord → FIFO (/home/radio/fifo/fifo<band>) → rx-websdr (:8095)
                                                        → браузер (HTML5)
```
...
тестовый сервер (см. ниже) ничего не ломает в основном.
...
- До 12 бэндов одновременно (и более), каждый со своей полосой и шагом.
